...
pub trait DataInputStreamExt {
    ...
    fn read_line_finish_utf8<P: IsA<AsyncResult>>(&self, result: &P) -> Result<(Option<String>, usize), Error>;
...
impl<O: IsA<DataInputStream> + IsA<glib::object::Object>> DataInputStreamExt for O {
    ...
    fn read_line_finish_utf8<P: IsA<AsyncResult>>(&self, result: &P) -> Result<(Option<String>, usize), Error> {
        unsafe {
            let mut length = mem::uninitialized();
            let mut error = ptr::null_mut();
            let ret = ffi::g_data_input_stream_read_line_finish_utf8(self.to_glib_none().0, result.to_glib_none().0, &mut length, &mut error);
